...
void EventLoop::registerClientSocketReadEvent(Event *e)
{
	...
	EV_SET(&(dummyEvent), e->getClientFd(), EVFILT_READ, EV_ADD, 0, 0, e);
...
{
	EV_SET(&(dummyEvent), e->getPipeFd(), EVFILT_READ, EV_ADD, 0, 0, e);
	if (kevent(this->kq_fd, &(dummyEvent), 1, NULL, 0, NULL) == -1) 
...
{
	EV_SET(&(dummyEvent), e->getFileFd(), EVFILT_READ, EV_ADD, 0, 0, e);
	if (kevent(this->kq_fd, &(dummyEvent), 1, NULL, 0, NULL) == -1) 
...
{
	//client socket을 읽기전용으로  kqueue에 등록
	EV_SET(&(dummyEvent), e->getClientFd(), EVFILT_WRITE, EV_ADD, 0, 0, e);
	if (kevent(this->kq_fd, &(dummyEvent), 1, NULL, 0, NULL) == -1) 
...
{
	EV_SET(&(dummyEvent), e->getPipeFd(), EVFILT_WRITE, EV_ADD, 0, 0, e);
	if (kevent(this->kq_fd, &(dummyEvent), 1, NULL, 0, NULL) == -1) 
...
{
	EV_SET(&(dummyEvent), e->getFileFd(), EVFILT_WRITE, EV_ADD, 0, 0, e);
	if (kevent(this->kq_fd, &(dummyEvent), 1, NULL, 0, NULL) == -1) 
